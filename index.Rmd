...
crime_by_year <- count(crime_pre, incident_year) %>%
  arrange(incident_year)

crime_by_year_type <- count(crime_pre, incident_year, incident_type_primary) %>%
  arrange(incident_year)

ggplot() +
  geom_col(data=crime_by_year_type, mapping=aes(incident_year, n, fill=incident_type_primary, group = 1)) +
...
From the plot above, obviously, it deomonstrates that the number of crime incidents has been decreasing through the period of time.
...
```{r, message=FALSE, warning=FALSE, fig.align='center'}
crime_by_hour <- count(crime_pre, incident_hour) %>%
  arrange(incident_hour)

crime_by_hour_type <- count(crime_pre, incident_hour, incident_type_primary) %>%
  arrange(incident_hour)

ggplot() +
  geom_col(data=crime_by_hour_type, mapping=aes(incident_hour, n, fill=incident_type_primary, group = 1)) +
  coord_polar(theta="x") +
...
anim <- ggplot(crime_by_year_type_ratio, aes(incident_type_primary, percentage, fill=incident_type_primary)) +
  geom_col() +
  coord_polar(theta="x") +
  theme_minimal() +
...
From the dynamic plot, we notice that the three leading types of crime are larceny/theft, burglary, and assault. The overall fluctuation of the ratios throughout the 10-year time period is not drastic. However, there is still some shift in terms of two types of crime, namely, assault and burglary.
...
leaflet() %>%
  ...
  addCircleMarkers(lng = crime_2019$longitude, 
                   lat = crime_2019$latitude,
                   radius = 2,
                   popup = paste0("Crime Type: ", crime_2019$incident_type_primary,"<br>",
                                  "Address: ", crime_2019$address),
